...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formData.password === 'Password@360'
    ) {
      toast.success('Admin login successful!', { position: 'top-center', autoClose: 2000, theme: 'colored' });
      setTimeout(() => {
        navigate('/admin-dashboard');
      }, 2000);
    } else {
      toast.error('Invalid admin credentials', { position: 'top-center', autoClose: 2500, theme: 'colored' });
    }
    setLoading(false);
  };
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <input
              ...
              readOnly
              className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm bg-gray-100 cursor-not-allowed"
